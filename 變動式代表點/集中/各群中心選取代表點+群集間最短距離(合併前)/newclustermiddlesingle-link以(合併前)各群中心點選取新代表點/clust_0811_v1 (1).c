...
void choose(nnode **nodes, int *next_item,
        ...
        //       printf("choose %d %d \n",best_a,best_b);
               ...
               int bestaitem[12000] ;
               int bestbitem[12000] ;
               float bestaitemdist[12000]={0} ;
               float bestbitemdist[12000]={0} ;
               int sumabitem[12000] = {0} ;
               float sumabdist[12000] = {0} ;
               float compareclu_dists[12000] ;
               int representiveitems[12000] ;
               ...
               float dist2[12000] ;
               float minditstance ;
               float bestamin[12000] ;
               float bestbmin[12000] ;
              // item_first_a = nodes[best_a]->first_item;
              // item_j = nodes[best_b]->first_item;
    /*   if(num_clusters_remaining  > 3){
        for(node_i = 0 ; node_i < num_clusters_remaining ; node_i++){
             printf("choose node_i %d" , node_i) ;
        for(i =0 ; i < floor(sqrt(nodes[node_i]->num_items)) ; i++){
             printf(" %d",arr[node_i][i]) ;
       ...
       for(i = 0 ; i < floor(sqrt(nodes[best_a]->num_items)) ; i++){
          ...
          if( minditstance > dist){
          minditstance = dist ;
           }
         //  printf("item %d item %d dist %f \n",item_i,item_j,dist);
          }
          bestamin[i] = minditstance ;
        //  printf("item %d mindistance %f\n",item_i,minditstance) ;
         }
       ...
       for(i = 0 ; i < floor(sqrt(nodes[best_a]->num_items)) ; i++){
        ...
        for(j = 0 ; j < floor(sqrt(nodes[best_a]->num_items)) ; j++){
          ...
          bestaitemdist[i] += dist ;
         // printf(" %d %f \n" ,item_j,dist) ;
          }
        bestaitemdist[i]+=bestamin[i] ;
       // printf("sumbestaitem %d %f \n", bestaitem[i],bestaitemdist[i]) ;
       }
       ...
       for(i = 0 ; i < floor(sqrt(nodes[best_b]->num_items)) ; i++){
        ...
        for(j = 0 ; j < floor(sqrt(nodes[best_b]->num_items)) ; j++){
          ...
          bestbitemdist[i] += dist ;
         // printf(" %d %f \n" ,item_j,dist) ;

        }

       // printf("sumbestbitem %d %f \n", bestbitem[i],bestbitemdist[i]) ;
        bestbitemdist[i]+=bestbmin[i] ;
       // printf("sumbestbitem %d %f \n", bestbitem[i],bestbitemdist[i]) ;
       }
      // system("pause") ;
   for(i = 0 ; i < floor(sqrt(nodes[best_a]->num_items))+ floor(sqrt(nodes[best_b]->num_items)); i++){
       if(i < floor(sqrt(nodes[best_a]->num_items))){
       sumabitem[i] = bestaitem[sum1] ;
       sumabdist[i] = bestaitemdist[sum1] ;
       sum1 +=1 ;
       //printf("%d %f \n ",sumabitem[i],sumabdist[i]) ;
       }else{
       sumabitem[i] = bestbitem[sum2] ;
       sumabdist[i] = bestbitemdist[sum2] ;
       sum2+=1 ;
       //printf("%d %f \n ",sumabitem[i],sumabdist[i]) ;
       }
}
   ...
   for(i =0;i < floor(sqrt(nodes[best_a]->num_items))+ floor(sqrt(nodes[best_b]->num_items)) ;i++){
      for(j =i; j < floor(sqrt(nodes[best_a]->num_items))+ floor(sqrt(nodes[best_b]->num_items));j++){
         if(sumabdist[j] < sumabdist[i]){
                temp = sumabdist[j];
                sumabdist[j] = sumabdist[i];
                sumabdist[i] = temp;
                temp2 = sumabitem[j] ;
