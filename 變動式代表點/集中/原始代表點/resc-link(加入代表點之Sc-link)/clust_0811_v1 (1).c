...
void choose(nnode **nodes, int *next_item,
   ...
   for(i =0;i < x ;i++){
      for(j =i; j < x;j++){
         if(compareclu_dists[j]<compareclu_dists[i]){
                temp = compareclu_dists[j];
                compareclu_dists[j] = compareclu_dists[i];
                compareclu_dists[i] = temp;
                temp2 = itembesta[j] ;
                itembesta[j] = itembesta[i];
                itembesta[i] = temp2;
                temp3 = itembestb[j] ;
                itembestb[j] = itembestb[i];
                itembestb[i] = temp3;
...
for(i =0;i < x ;i++){
     //   printf(" %d %d %f\n",itembesta[i],itembestb[i],compareclu_dists[i]);
 }
...
for(k =0; k < r ; k++){
  ...
  for(i =0; i < r ; i++){
      ...
      representiveitems[index] = data[i] ;
     // printf("re %d " ,representiveitems[index]) ;
      index = index +1 ;
      }
      tt=0 ;
  }
