...
void print_nodes(nnode **nodes, int *next_item, int num_clusters_remaining) {
  ...
  FILE *fp;
  fp = fopen("skew.txt","a");
  average = 1424 / num_clusters_remaining ;
...
void choose(nnode **nodes, int *next_item,
        ...
        //       printf("choose %d %d \n",best_a,best_b);
               ...
               float sumbestaandbestb[10000] = {0} ;
               int sum1=0,sum2=0 ;
    ...
    /*   if(num_clusters_remaining  > 3){
     ...
     /*   for (j = 0; j < floor(sqrt(nodes[best_b]->num_items)); j++) {
            ...
            for (i = 0 ; i < floor(sqrt(nodes[best_a]->num_items)); i++) {
                    ...
                    y=y+1 ;
                    a=a+1 ;
                 //   printf("sum besta i %d %f\n",i,sumbesta[i]) ;
          } // i loop; items in best_a
         // printf("sum bestb j %d %f\n",j,sumbestb[j]) ;
          t = t+1 ;
        }*/
       //  printf("num %f \n",floor(sqrt(nodes[best_b]->num_items))+floor(sqrt(nodes[best_a]->num_items)));
         for(i = 0 ; i < floor(sqrt(nodes[best_b]->num_items))+floor(sqrt(nodes[best_a]->num_items)); i++){
         if(i < floor(sqrt(nodes[best_a]->num_items))){
         sumabitem[i] = arr[best_a][sum1] ;
         sum1 +=1 ;
     //    printf(" %d",sumabitem[i]) ;
         }else{
         sumabitem[i] = arr[best_b][sum2];
         sum2+=1 ;
     //    printf(" %d",sumabitem[i]) ;
         }
      }
   //     printf("\n") ;
        for(i = 0 ; i < floor(sqrt(nodes[best_b]->num_items))+floor(sqrt(nodes[best_a]->num_items)) ; i ++){
        item_i = sumabitem[i] ;
        for(j = 0 ; j < floor(sqrt(nodes[best_b]->num_items))+floor(sqrt(nodes[best_a]->num_items)); j++){
        item_j = sumabitem[j] ;
        if (j!=i){
        if (item_i < item_j){
                       dist = item_distances[item_i * num_items + item_j];}
        else {dist = item_distances[item_j * num_items + item_i];}
        sumbestaandbestb[i] += dist ;
      //  printf("%f \n",dist) ;
        }
       }
   ...
   for(i =0;i < floor(sqrt(nodes[best_b]->num_items))+floor(sqrt(nodes[best_a]->num_items)) ;i++){
      for(j =i; j < floor(sqrt(nodes[best_b]->num_items))+floor(sqrt(nodes[best_a]->num_items));j++){
