 ...
 //  item_first_a = nodes[best_a]->first_item ;
   ...
   for (node_i = 0; node_i < best_a; node_i++) {
      ...
      // item_j = nodes[node_i]->first_item ;
       ...
       for (j = 0; j < floor(sqrt(nodes[node_i]->num_items)); j++) {
          for (i = 0, item_i = item_first_a; i < floor(sqrt(nodes[best_a]->num_items)); i++) {
              item_i = arr[best_a][i] ;
              item_j = arr[node_i][j] ;
              if (item_i < item_j)
...
//   float sum;
   ...
   for (i = 0; i < num_clusters; i++) {
       ...
       for (attr_i = 0; attr_i < NUM_ATTRS; attr_i++){
            ...
            cents[i].coord[attr_i] /= (float)num_items;
        //    printf("%f \n",cents[i].coord[attr_i]) ;
           }
...
*/
	...
	for (i_attr = 0; i_attr < num_attrs; i_attr++) {
	  ...
	  sum_sq_or_sd[i_attr] = sqrt(sum_sq_or_sd[i_attr] / (float)num_items- sum_or_mean[i_attr] * sum_or_mean[i_attr]);
	}
...
void choose(nnode **nodes, int *next_item,
        ...
        //       printf("choose %d %d \n",best_a,best_b);
               ...
               int itembesta[10000] ;
               int itembestb[10000] ;
               float compareclu_dists[10000] ;
               int representiveitems[10000] ;
               ...
               float choosea[10000] ;
               float chooseb[10000] ;
               int   chooseitema[10000] ;
               int chooseitemb[10000] ;
               int bestam,bestbm ;
               int aitem ,bitem ;
               int xxx ;
              // item_first_a = nodes[best_a]->first_item;
        ...
        for (j = 0; j < floor(sqrt(nodes[best_b]->num_items)); j++) {
                    item_j = arr[best_b][j] ;
                for (i = 0 ; i < floor(sqrt(nodes[best_a]->num_items)); i++) {
                    item_i = arr[best_a][i] ;
                    if (item_i < item_j)
                       clu_dist = item_distances[item_i * num_items + item_j];
                    else clu_dist = item_distances[item_j * num_items + item_i];
                    if (item_i < item_j)
                       dist = item_distances[item_i * num_items + item_j];
                    else dist = item_distances[item_j * num_items + item_i];
                 //   printf("choose best_a %d to %d: item %d %d %f %f\n", best_a, best_b, item_i, item_j, dist, clu_dist);
                    compareclu_dists[a] = dist ;
                    itembesta[x] = item_i ;
                    itembestb[y] = item_j ;
                    x=x+1 ;
                    y=y+1 ;
         ...
         for(i = 0 ; i < floor(sqrt(nodes[best_a]->num_items))  ; i++){
           ...
           for(j = 0 ; j < floor(sqrt(nodes[best_b]->num_items)) ; j++){
            ...
            // printf("item %d item %d %f \n", item_j,item_i,dist ) ;
                if(j == 0){
                choosea[i] = dist ;
                }else if(choosea[i] > dist ){
                choosea[i] = dist ;
                }
             }
             chooseitema[i] = item_j ;
          //   printf("item %d min %f \n",chooseitema[i],choosea[i]) ;
            }


         for(i = 0 ; i < floor(sqrt(nodes[best_b]->num_items))  ; i++){
            item_j = arr[best_b][i] ;
           for(j = 0 ; j < floor(sqrt(nodes[best_a]->num_items)) ; j++){
             item_i = arr[best_a][j] ;
             if (item_i < item_j){
                       dist = item_distances[item_i * num_items + item_j];
             }else{
            ...
            // printf("item %d item %d %f \n", item_j,item_i,dist ) ;
                if(j == 0){
                chooseb[i] = dist ;
                }else if(chooseb[i] > dist ){
                chooseb[i] = dist ;
                }
             }
             chooseitemb[i] = item_j ;
            // printf("item %d min %f \n",chooseitemb[i],chooseb[i]) ;
            }
            aitem = nodes[best_a]->num_items ;
            bitem = nodes[best_b]->num_items ;
          //  printf("a-item %f bitem %f %f\n",floor(sqrt(aitem)),floor(sqrt(bitem)),floor(sqrt(bitem+aitem))) ;
          //  printf("a-item %f \n",round(floor(sqrt(aitem+bitem))*floor(sqrt(aitem))/(floor(sqrt(aitem))+floor(sqrt(bitem))))) ;
          //  printf("b-item %f \n",round(floor(sqrt(aitem+bitem))*floor(sqrt(bitem))/(floor(sqrt(aitem))+floor(sqrt(bitem))))) ;
            aitem = round(floor(sqrt(aitem+bitem))*floor(sqrt(aitem))/(floor(sqrt(aitem))+floor(sqrt(bitem)))) ;
            bitem = round(floor(sqrt(aitem+bitem))*floor(sqrt(bitem))/(floor(sqrt(aitem))+floor(sqrt(bitem)))) ;
           // printf("aitem %d bitem %d \n",aitem,bitem) ;
           // printf("%f \n",floor(sqrt(bitem+aitem))) ;
            xxx = floor(sqrt(bitem+aitem)) ;
            if(aitem == bitem){
              aitem = 0 ;
              bitem = 0 ;
              for(i = 0; i <  xxx; i++ ){
            ...
            //  printf("aitem %d bitem %d \n",aitem,bitem) ;
            for(i = 0; i < floor(sqrt(nodes[best_a]->num_items)); i++ ){
                for(j = i; j < floor(sqrt(nodes[best_a]->num_items)); j++){
                        if(choosea[j] < choosea[i]){
                          temp = choosea[j];
                          choosea[j] = choosea[i] ;
                          choosea[i] = temp ;
                          temp = chooseitema[j] ;
                          chooseitema[j] =chooseitema[i] ;
                          chooseitema[i] = temp ;
                        }
                }
              //  printf("itema %d %f \n",chooseitema[i],choosea[i]) ;
            ...
            for(i = 0; i < floor(sqrt(nodes[best_b]->num_items)); i++ ){
                for(j = i; j < floor(sqrt(nodes[best_b]->num_items)); j++){
                        if(chooseb[j] < chooseb[i]){
                          temp = chooseb[j];
                          chooseb[j] = chooseb[i] ;
                          chooseb[i] = temp ;
                          temp = chooseitemb[j] ;
                          chooseitemb[j] =chooseitemb[i] ;
                          chooseitemb[i] = temp ;
                        }
                }
            //    printf("itema %d %f \n",chooseitemb[i],chooseb[i]) ;
...
//printf("choose \n") ;
  ...
  rr = nodes[best_a]->num_items + nodes[best_b]->num_items ;
  //printf("\n") ;
  //printf("chooose %d \n",rr) ;
  rr = floor(sqrt(rr)) ;
...
//                int num_items = process_input(&items, argv[1]);
              ...
              //   num_items = process_input(&items, "frog.txt");
#else
